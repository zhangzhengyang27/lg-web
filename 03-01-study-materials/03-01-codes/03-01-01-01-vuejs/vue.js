...
(function (global, factory) {
    ...
    // 由于它们的显式性和函数内联，这些助手在 JS 引擎中生成了更好的 vm 代码
    ...
    // Quick object check - this is primarily used to tellObjects from primitive values when we know the valueis a JSON-compliant type.
    // 快速对象检查 - 当我们知道原始值是符合 JSON 的类型时，这主要用于从原始值中区分对象。
    // 判断是否是对象
    ...
    //类型判断 返回Array ，Function，String,Object,Re 等
    ...
    /*
     ...
     用于不支持它的环境的简单绑定 polyfill...例如PhantomJS 1.x。从技术上讲，我们不再需要这个，
     因为原生绑定现在在大多数浏览器中性能更高，但是删除它会破坏能够在 PhantomJS 1.x 中运行的代码，因此必须保留它以实现向后兼容性
     */
    ...
    function looseEqual(a, b) {
        ...
        } else if (!isObjectA && !isObjectB) {  //b和a 都不是对象的时候 把a和b变成字符串，判断他们是否相同
    ...
    var config = ({
        ...
        // Whether to record perf 是否记录PERF
        performance: false,

        // Error handler for watcher errors  监视器错误的错误处理程序
        errorHandler: null,


        // Warn handler for watcher warns 观察加警告处理。
        warnHandler: null,
        ...
        // Parse the real tag name for the specific platform. 解析真实的标签平台
    ...
    // Browser environment sniffing 判断设备和浏览器
...

    {
        ...
        // 函数参数 ： msg： 警告信息， vm：vue对象  函数返回值 ： void
        warn = function (msg, vm) {
            //vm 如果没有传进来就给空， 不然执行generateComponentTrace 收集 vue错误码
        ...
        formatComponentName = function (vm, includeFile) {
            ...
            // name 组件名称或者是文件名称
            /*
             * classify 去掉-_连接  大写字母连接起来
             * 如果name存在则返回name
             * 如果name不存在那么返回‘<Anonymous>’+ 如果file存在并且includeFile！==false的时候 返回" at " + file
             *
             */
            return (
        ...
        // 生成组建跟踪 vm=vm.$parent递归收集到msg出处。
        // 这个方法没看懂
        generateComponentTrace = function (vm) {
            // 如果_isVue 等于真，并且有父亲节点的
            if (vm._isVue && vm.$parent) {
                ...
                //如果i是0 则输出 ‘---->’  如果i 不是0的时候输出组件名称 tree.map()
                return '\n\nfound in\n\n' + tree
                    .map(function (vm, i) {
    ...
    // A dep is an observable that can have multiple depdirectives subscribing to it.是可观察到的，可以有多个订阅它的指令。
    // 主题对象Dep构造函数  主要用于添加发布事件后，用户更新数据的响应式原理之一函数
    ...
    // 已弃用:用于向后兼容的 componentInstance 的别名
    ...
    // optimized shallow clone used for static nodes and slot nodes because they may be reused across
    // multiple renders, cloning them avoids errors when DOM manipulations rely on their elm reference.
    // 用于静态节点和槽节点的优化浅克隆，因为它们可以在多个渲染中重复使用，当 DOM 操作依赖于它们的 elm 引用时，克隆它们可以避免错误。
    ...
    // 不检查此文件，因为flow不能很好地与 Array 原型上的动态访问方法配合使用
    ...
    /*
     Observer class that is attached to each observed object. Once attached, the observer converts the target
     object's property keys into getter/setters that collect dependencies and dispatch updates.
     附加到每个观察对象的观察者类。附加后，观察者将目标对象的属性键转换为收集依赖项和调度更新的 getter setter
     每个观察到的观察者类对象。一旦被连接，观察者就转换目标。
     对象的属性键为吸收器/设置器收集依赖关系并发送更新。
    ...
    // 遍历每个属性并将它们转换为 getter/setter。仅当值类型为 Object 时才应调用此方法。
    ...
    /*
     ...
     尝试为一个值创建一个观察者实例，如果成功观察则返回新观察者，如果该值已经有一个观察者，则返回现有观察者。
    ...
    function observe(value, asRootData) {
        ...
        if (hasOwn(value, '__ob__') && value.__ob__ instanceof Observer) {
            console.log('hasOwn value' + value)

            ob = value.__ob__;
        } else if (shouldObserve && !isServerRendering() && (Array.isArray(value) || isPlainObject(value)) && Object.isExtensible(value) && !value._isVue) {
            // shouldObserve 为真  并且不是在服务器node环境下 是数组或者是对象 _isVue为假
            // Object.preventExtensions(O) 方法用于锁住对象属性，使其不能够拓展，也就是不能增加新的属性，但是属性的值仍然可以更改，也可以把属性删除，Object.isExtensible用于判断对象是否可以被拓展
        ...
        // vmCount是用来记录此Vue实例被使用的次数的， 比如，我们有一个组件logo，页面头部和尾部都需要展示logo，都用了这个组件，那么这个时候vmCount就会计数，值为2
     ...
     * 添加观察者 get set方法
     * 参数的意义：对象、对象的key、监听的数据 返回的数据、日志函数、是否要添加__ob__ 属性
     */
    // 没看懂 ？？？
    function defineReactive(obj, key, val, customSetter, shallow) {
        ...
        // cater for pre-defined getter/setters  迎合预定义的 getter/setter
    ...
    // 在对象上设置属性。添加新属性和 触发器更改通知，如果该属性不存在。
    // 如果是数组  并且key是数字 就更新数组
    // 如果是对象则重新赋值
    // 如果 (target).__ob__ 存在则表明该数据以前添加过观察者对象中  //通知订阅者ob.value更新数据 添加观察者  define  set get 方法
    function set(target, key, val) {
        // 判断数据 是否是undefined或者null 判断数据类型是否是string，number，symbol，boolean ;必须是对象数组才可以 否则发出警告
        ...
        if (Array.isArray(target) && isValidArrayIndex(key)) {
            ...
            // 向数组尾部添加一个新数据，相当于push
            target.splice(key, 1, val);
            return val
        }
        //判断key是否在target上，并且不是在Object.prototype 原型上，而不是通过父层原型链查找的
    ...
    // 当数组被触摸时收集对数组元素的依赖，因为我们不能像属性获取器那样拦截数组元素的访问。
    ...
    /**
     * Data
     * mergeDataOrFn递归合并数据 深度拷贝。如果vm不存在，并且childVal不存在就返回parentVal。如果vm不存在并且parentVal不存在则返回childVal。
     * 如果vm不存在parentVal和childVal都存在则返回mergedDataFn。如果vm存在则返回 mergedInstanceDataFn函数
     */
    function mergeDataOrFn(parentVal, childVal, vm) {
        ...
        if (!vm) {
            ...
            // when parentVal & childVal are both present,we need to return a function that returns the merged result of both functions...
            // 当 parentVal 和 childVal 都存在时，我们需要返回一个函数，该函数返回两个函数的合并结果...
            // no need to check if parentVal is a function here becauseit has to be a function to pass previous merges.
    ...
    /*
     ...
     当 vm 存在时（实例创建），我们需要在构造函数选项、实例选项和父选项之间进行三向合并。
     创建一个res对象，获取parentVal对象中的数据。如果parentVal存在则获取parentVal对象的数据存在res中的  __props__ 中，如果没有则创建一个空的对象。
     */
    ...
    function validateComponentName(name) {
        if (!/^[a-zA-Z][\w-]*$/.test(name)) {
            warn(
                ...
                'can only contain alphanumeric characters and the hyphen, ' + 'and must start with a letter.'
            );
        }
     ...
     * normalizeProps 检查 props 数据类型，并把type标志打上。如果是数组循环props属性数组，如果val是string则把它变成驼峰写法  res[name] = {type: null}; 。
     * 如果是对象也循环props把key变成驼峰，并且判断val是不是对象如果是对象则    res[name] 是{type: val}否则    res[name] 是val。
     */
    ...
    function normalizeDirectives(options) {
        ...
        if (dirs) {
            for (var key in dirs) {
                ...
                if (typeof def === 'function') { //如果是函数为该函数添加一个对象和值
    ...
    function mergeOptions(parent, child, vm) {
        ...
        //如果子组件有mixins数组则也递归合并，继承方式 mixins 必须是数组
        if (child.mixins) {
        ...
        // strats类里面的方法都是合并数据 如果没有子节点childVal，就返回父节点parentVal，如果有子节点childVal就返回子节点childVal。
     ...
     * 检测指令是否在组件对象上面 ,返回注册指令或者组建的对象, 包括检查directives，filters，components
     * options, //参数
    ...
    function resolveAsset(options, type, id, warnMissing) {
        ...
        // 首先检查本地注册的变化 检查id是否是 assets 实例化的属性或者方法
        if (hasOwn(assets, id)) {
            return assets[id]
        }
     ...
     * 校验 props 参数 就是组件定义的props 类型数据，校验类型
     * 判断 prop.type的类型是不是Boolean或者String，如果不是他们两类型，调用getPropDefaultValue获取默认值并且把value添加到观察者模式中
     * key   key
     * propOptions  原始props 参数
     * propsData    转义过的组件props数据
     * vm           VueComponent 组件构造函数  this属性
     */
    function validateProp(key, propOptions, propsData, vm) {
        ...
        // Boolean 传一个布尔值，但是一般是函数或者数组函数才有意义，而且是函数声明的函数并不是 函数表达式prop.type 也需要是函数
        // 返回的是相同的索引  判断属性类型定义的是否是Boolean
        var booleanIndex = getTypeIndex(Boolean, prop.type);
        if (booleanIndex > -1) {  //如果是boolean值
            if (absent && !hasOwn(prop, 'default')) { //如果key不是propsData实例化，或者没有定义default默认值的时候， 设置value为false
                value = false;
            } else if (value === '' || value === hyphenate(key)) {
                // 如果value是空或者key转出 - 形式和value 相等的时候
                // only cast empty string / same name to boolean if boolean has higher priority
                // 如果布尔值具有更高的优先级，则仅将空字符串同名转换为布尔值
                // 判断prop.type的类型是否是string字符串类型
        ...
        // check default value 检查默认值 如果没有值 value 也不是boolean， 也不是string的时候
        if (value === undefined) {
            // 有可能是函数
            value = getPropDefaultValue(vm, prop, key);

            // since the default value is a fresh copy,make sure to observe it. 由于默认值是一个新的副本，一定要遵守。
        ...
        {
            //检查prop是否合格   属性的type值、props属性中的key、 view属性的值、 VueComponent组件构造函数、false
    ...
    function getPropDefaultValue(vm, prop, key) {
        ...
        // 判断该对象prop中的default是否是prop实例化的  no default, return undefined
        ...
        //原始PROP值也未从先前的渲染中定义，返回先前的默认值以避免不必要的监视触发器
        ...
        // 非功能类型调用工厂函数,一个值是函数，即使它的原型在不同的执行上下文中也是函数。
    ...
    function assertType(value, type) {
        ...
        // 检测改函数是什么类型  type 必须是String|Number|Boolean|Function|Symbol 构造函数 这里才为真 (String|Number|Boolean|Function|Symbol)
    ...
    //判断expectedTypes中的函数和 type 函数是否有相等的;如有有则返回索引index 如果没有则返回-1
    ...
    /*
     ...
     handleError 方法中首先获取到报错的组件，之后递归查找当前组件的父组件，依次调用 errorCaptured 方法。
     在遍历调用完所有 errorCaptured 方法、或 errorCaptured 方法有报错时，会调用 globalHandleError 方法。
     globalHandleError 方法调用了全局的 errorHandler 方法。
     如果 errorHandler 方法自己又报错了呢？生产环境下会使用 console.error 在控制台中输出。

     可以看到 errorCaptured 和 errorHandler 的触发时机都是相同的，不同的是 errorCaptured 发生在前，且如果某个组件的 errorCaptured 方法返回了 false，
     那么这个异常信息不会再向上冒泡也不会再调用errorHandler 方法。
     */
    ...
    // 在这里，我们有使用微任务和（宏）任务的异步延迟包装器。在 < 2.4 中，我们在所有地方都使用了微任务，但是在某些情况下，微任务的优先级太高，
    // 并且在所谓的顺序事件（例如 4521、6690）之间甚至在同一事件的冒泡之间（6566）触发。然而，当状态在重绘之前改变时（例如 6813，out-in 转换），
    // 在任何地方使用（宏）任务也会有微妙的问题。这里我们默认使用微任务，但在需要时公开一种强制（宏）任务的方法（例如，在由 v-on 附加的事件处理程序中）。
    ...
    // 确定（宏）任务延迟实施。从技术上讲 setImmediate 应该是理想的选择，但它只在 IE 中可用。
    // 在同一个循环中触发的所有 DOM 事件之后，唯一始终将回调排队的 polyfill 是使用 MessageChannel。
    /* istanbul ignore if */
    ...
    if (typeof Promise !== 'undefined' && isNative(Promise)) {
        ...
        microTimerFunc = function () {
            ...
            // 在有问题的 UIWebViews 中，Promise.then 并没有完全中断，但它可能会陷入一种奇怪的状态，其中回调被推入微任务队列但队列没有被刷新，
            // 直到浏览器需要做一些其他工作，例如处理一个计时器。因此，我们可以通过添加一个空计时器来“强制”刷新微任务队列。
            //如果是ios 执行下 noop 空函数
            if (isIOS) {
                setTimeout(noop);
            }
     ...
     */
    // 包装一个函数，如果内部的任何代码触发状态改变，使用宏（宏）任务而不是微任务对这些队列进行排队
    ...
    //为callbacks 收集队列cb 函数 并且根据 pending 状态是否要触发callbacks 队列函数  ctx this的指向
    function nextTick(cb, ctx) {
        ...
        if (!pending) {
            ...
            // 执行异步宏任务
            if (useMacroTask) {
                macroTimerFunc(); //异步触发或者实现观察者触发callbacks队列中的函数
            } else {
                microTimerFunc(); //异步触发或者实现观察者触发callbacks队列中的函数
            }
        }
        ...
        */
        ...
        if (hasProxy) {
            ...
            config.keyCodes = new Proxy(config.keyCodes, {
                set: function set(target, key, value) {
                    // 匹配键盘上的快捷键 'stop,prevent,self,ctrl,shift,alt,meta,exact'
        ...
        var hasHandler = {
            has: function has(target, key) {
                ...
                //是否含有全局api 就是window 的内置函数全局api
                var isAllowed = allowedGlobals(key) || key.charAt(0) === '_';
                //如果 key 在target对象中不存在 或者 isAllowed 不是全局api 并且第一个字符不是_的时候发出警告
                if (!has && !isAllowed) {
                    //不存在key发出警告
    ...
    function _traverse(val, seen) {
        ...
        if (val.__ob__) {
            ...
            // seen 是 seenObjects = new _Set(); add 就是set对象中的add方法，添加唯一的值得key
            seen.add(depId);
        }
        //如果是数组
        if (isA) {
    ...
    */
    ...
    // createFnInvoker，如果事件只是个函数就为事件添加多一个静态类， invoker.fns = fns; 把真正的事件放在fns。而 invoker 则是转义fns然后再运行fns
    function createFnInvoker(fns) {
        function invoker() {
            ...
            if (Array.isArray(fns)) {
                ...
                // 执行fns数组中的函数，并且把 invoker arguments$1参数一个个传给fns 函数中
                for (var i = 0; i < cloned.length; i++) {
                    cloned[i].apply(null, arguments$1);
                }
            } else {
                // return handler return value for single handlers
                // 如果fns不是数组函数，而是一个函数，则执行arguments$1参数一个个传给fns 函数中
    ...
    */
    ...
    /*
     ...
     从 props属性中获取vnode数据
     循环propOptions对象，把驼峰的key转换成横杠的key。
     校验props属性的key是否和attrs属性值相同，如果相同删除掉attrs属性的同样key的值。
     获取props属性的值添加搞res对象中，返回出去
     data  -tag标签属性数据
    ...
    function extractPropsFromVNodeData(data, Ctor, tag) {
        ...
        // 我们只是在这里提取原始值,验证和默认值在子组件本身中处理

        // 获取Ctor 参数中的 props,获取组件的props属性
    ...
    */
    ...
    // 对于纯 HTML 标记，可以完全跳过规范化，因为生成的渲染函数保证返回 Array<VNode>。有两种情况需要额外的标准化：
    ...
    // 当子组件包含组件时-因为功能组件可能返回一个数组而不是单个根。在这种情况下，只需要一个简单的规范化——如果任何孩子是一个数组，
    // 我们用 Array.prototype.concat 将整个东西展平。它保证只有 1 级深度，因为功能组件已经规范化了它们自己的子级
    ...
    // 2.当子组件包含总是生成嵌套数组的结构时，例如<template>、<slot>、v-for，
    // 或者当子组件由用户提供手写渲染函数 JSX 时。在这种情况下，需要完全规范化以迎合所有可能类型的子值
    ...
    function normalizeChildren(children) {
        ...
        return isPrimitive(children) ?
            [createTextVNode(children)] : Array.isArray(children) ? normalizeArrayChildren(children) : undefined
    }
    ...
    /*
     ...
     children 表示要规范的子节点
     nestedIndex 表示嵌套的索引，
     主要的逻辑就是遍历 children，获得单个节点，然后类型判断，
     如果是一个数组类型，则递归调用 normalizeArrayChildren;
     如果是基础类型，则通过 createTextVNode 方法转换成 VNode 类型；
     否则就已经是 VNode 类型了，如果 children是一个列表并且列表还存在嵌套的情况，则根据 nestedIndex去更新它的 key。
     这里需要注意一点，在遍历的过程中，
     对这 3 种情况都做了如下处理：如果存在两个连续的 text 节点，会把它们合并成一个 text节点。
     因为单个child可能是一个数组类型。把这个深层的数组遍历到一层数组上面去。如果是深层数组则调用递归normalizeArrayChildren
     */
        ...
        for (i = 0; i < children.length; i++) {
            ...
            if (Array.isArray(c)) {
                if (c.length > 0) {
                    ...
                    // merge adjacent text nodes 合并相邻文本节点 如果c[0] 中的第一个是文本节点 并且 res 最后一个节点是 文本节点
                ...
                if (isTextNode(last)) {
                    ...
                    // this is necessary for SSR hydration because text nodes are essentially merged when rendered to HTML strings
                    // 这对于 SSR 作用是必要的，因为文本节点在呈现为 HTML 字符串时本质上是合并的
                    res[lastIndex] = createTextVNode(last.text + c);
                } else if (c !== '') {
                    // convert primitive to vnode  转换成 vnode 创建 文本节点
                    res.push(createTextVNode(c));
                }
            } else {
                //如果c 中的第一个是文本节点 并且 res 最后一个节点是 文本节点
                if (isTextNode(c) && isTextNode(last)) {
                    // merge adjacent text nodes 合并相邻文本节点 创建文本节点
                    res[lastIndex] = createTextVNode(last.text + c.text);
                } else {
                    // default key for nested array children (likely generated by v-for)
                    // 嵌套数组子项的默认键（可能由 v-for 生成）
                    // 如果children._isVList 为true、c.tag 不为空、c.key 为空的时候、nestedIndex不为空
    ...
    function ensureCtor(comp, base) {
        ...
        // 如果 comp.__esModule 存在或者 支持hasSymbol 类型 并且判断 对象类的标签属性是Module "[object Module]"
    ...
    /*
    createAsyncPlaceholder 创建简单的占位符 创建一个节点，解决异步组件
    factory, 工厂
    data,    数据
    context, 语境
    children 子节点
    ...
    function resolveAsyncComponent(factory, baseCtor, context) {
        ...
        if (isDef(factory.contexts)) {
            ...
            var resolve = once(function (res) {
                ...
                //仅当这不是同步解析时才调用回调(异步解析在SSR期间以同步的方式进行调整)
                if (!sync) {
                    //渲染组件更新数据
                    forceRender();
                }
            ...
            if (isObject(res)) {
                ...
                if (typeof res.then === 'function') {
                    // () => Promise 执行 promise 没有定义 resolved 成功
    ...
    */
        ...
        */
        Vue.prototype.$on = function (event, fn) {
            ...
            if (Array.isArray(event)) {
                ...
                // 如果是 hook: 开头的标记为vue vue系统内置钩子函数 比如vue生命周期函数等
    ...
    function initLifecycle(vm) {
        ...
        //判断parent是否是顶层root,如果是则$root赋值给$root
    ...
    function lifecycleMixin(Vue) {
        ...
        Vue.prototype._update = function (vnode, hydrating) {
            ...
            // Vue.prototype.__patch__ 基于使用的渲染后端注入入口点
            ...
            if (!prevVnode) {
                ...
                // 初始补丁后不需要 ref 节点，这可以防止将分离的 DOM 树保留在内存中 (#5851)
                vm.$options._parentElm = vm.$options._refElm = null;
            } else {
                //如果这个prevVnode存在，表示vnode的已经创建过，只是更新数据而已
    ...
    function mountComponent(vm, el, hydrating) {
        ...
        if ("development" !== 'production' && config.performance && mark) {
            ...
            updateComponent = function () {
                ...
                vm._update(
                    /*
                     render是虚拟dom，需要执行的编译函数 类似于这样的函数
                     (function anonymous( ) {
                        with(this){
                            return _c('div',{attrs:{"id":"app"}},[_c('input',{directives:[{name:"info",rawName:"v-info"},
                                    {name:"data",rawName:"v-data"}],attrs:{"type":"text"}}),_v(" "),_m(0)])
                           }
                     })
                     */
        ...
        // 我们在 watcher 的构造函数中将其设置为 vm._watcher，因为 watcher 的初始补丁可能会调用$forceUpdate（例如，在子组件的挂载钩子内），这依赖于 vm._watcher 已经定义
        /*
        创建观察者
        vm    --vm vode
    ...
    */
    function updateChildComponent(vm, propsData, listeners, parentVnode, renderChildren) {
        ...
        // update vm's placeholder node without re-render 无需重新渲染即可更新vm的占位符节点
        ...
        // these are also reactive so they may trigger child update if the child used them during render
        // 更新$attrs和$listener散列
        // 这些也是反应性的，因此如果孩子在渲染期间使用它们，它们可能会触发孩子更新
        ...
        if (propsData && vm.$options.props) {
            ...
            for (var i = 0; i < propKeys.length; i++) { //循环props属性
                ...
                /*
                 验证支柱,验证prosp是否是规范数据,并且为props添加value.__ob__ 属性，把prosp添加到观察者中
                 校验props参数就是组件定义的props类型数据，校验类型
        ...
        //解决插槽+强制更新如果有 子节点
        if (hasChildren) {
            //判断children有没有分发式插槽,并且过滤掉空的插槽,并且收集插槽
    ...
    // 判断是否有不活跃的组件禁用他 如果有活跃组件则触发钩子函数activated
    // direct 布尔值
    function activateChildComponent(vm, direct) {
        ...
        // 如果 _inactive=true不活跃组件 或者 vm._inactive === null
    ...
    // 循环子组件和父组件,判断是否有禁止的组件,如果有活跃组件则执行生命后期函数deactivated
    ...
    function flushSchedulerQueue() {
        ...
        // 在刷新之前对队列进行排序。这可确保：
        // 1. 组件从父组件更新到子组件。 （因为 parent 总是在 child 之前创建）
        // 2. 组件的用户观察者在其渲染观察者之前运行（因为用户观察者是在渲染观察者之前创建的）
        // 3. 如果组件在父组件的观察者运行期间被销毁，它的观察者可以被跳过。
        ...
        // 不要缓存长度，因为在我们运行现有的观察者时可能会推送更多的观察者
     ...
     * 将补丁期间激活的kept-alive组件排队。整个树被修补后，队列将被处理
     * 添加活跃的组件函数 把活跃的vm添加到activatedChildren 中
     */
    function queueActivatedComponent(vm) {
        // setting _inactive to false here so that a render function can
        // rely on checking whether it's in an inactive tree (e.g. router-view)
        // 在这里将_inactive设置为false,以便渲染函数可以依赖检查它是否在非活动树中（例如路由器视图）
        vm._inactive = false;
     ...
     * 将观察者推入观察者队列。除非在刷新队列时将其推送，否则将跳过具有重复 ID 的作业
     */
    function queueWatcher(watcher) {
        ...
        if (has[id] == null) {
            ...
            if (!waiting) {
                ...
                nextTick(
                    flushSchedulerQueue//更新观察者 运行观察者watcher.run() 函数,并且调用组件更新和激活的钩子
                );
            }
        }
    }
    ...
    var Watcher = function Watcher(vm, expOrFn, cb, options, isRenderWatcher) {
        console.log('====Watcher====')
        this.vm = vm;
        //是否是已经渲染过得观察者,把当前Watcher对象赋值给vm._watcher上
        if (isRenderWatcher) {
            vm._watcher = this;
        }
        //把观察者添加到队列里面 当前Watcher添加到vue实例上
        ...
        if (typeof expOrFn === 'function') {
            ...
            // path 因该是路由地址
            //  匹配上返回 true,   var bailRE = /[^\w.$]/;  匹配不是数字字母下划线 $符号 开头的为true
            if (bailRE.test(path)) {
                return
            }
        ...
        // lazy为真的的时候才能获取值  这个有是组件才为真
        // 计算getter，并重新收集依赖项。 获取值
    ...
    Watcher.prototype.addDep = function addDep(dep) {
        ...
        if (!this.newDepIds.has(id)) {//如果id不存在
            ...
            if (!this.depIds.has(id)) {  //如果depIds 不存在id则添加一个addSub
                dep.addSub(this);
            }
        }
    };
    ...
    Watcher.prototype.run = function run() {
        if (this.active) { //活跃
            ...
            // 即使值相同，Object/Arrays 上的深层观察者和观察者也应该触发，因为值可能已经发生了变异
            // 如果值不相等 或者值的object  获取deep为true
     ...
     * 依赖于此监视程序收集的所有dep。
     * 循环deps收集, newDeps dep 当newDeps 数据被清空的时候重新收集依赖
     */
    ...
    Watcher.prototype.teardown = function teardown() {
        var this$1 = this;
        if (this.active) {
            // remove self from vm's watcher list this is a somewhat expensive operation so we skip it
    ...
    function initProps(vm, propsOptions) {
        ...
        var loop = function (key) {
            ...
            var value = validateProp(key, propsOptions, propsData, vm);
            {
                ...
                //通过defineProperty的set方法去通知notify()订阅者subscribers有新的值修改,function是回调函数
            ...
            // instantiation here.
            // 在 Vue.extend() 期间，静态 props 已经代理在组件的原型上。我们只需要在此处代理实例化时定义的 props
    ...
    // 初始化数据,获取options.data 的数据,将他们添加到监听者中
    function initData(vm) {
        ...
        data = vm._data = typeof data === 'function' ? getData(data, vm) : data || {}; //直接获取数据
    ...
    function initComputed(vm, computed) {
        ...
        for (var key in computed) {
            ...
            // component prototype. We only need to define computed properties defined at instantiation here.
    ...
    function defineComputed(target, key, userDef) {
        ...
        if (typeof userDef === 'function') { // 属性的值如果是个函数
            sharedPropertyDefinition.get = shouldCache
                ? createComputedGetter(key) //如果不是node服务器是浏览器,创建计算属性 获取值 收集 dep 依赖
        ...
        if ("development" !== 'production' && sharedPropertyDefinition.set === noop) { //如果设置值等于一个空函数则警告
    ...
    //初始化事件Methods 把事件冒泡到 vm[key], 虚拟dom最外层中
    ...
    */
     ...
     *  添加slots插槽渲染方法,重写this._c、createElement 函数,渲染vonde
    ...
    function FunctionalRenderContext(data, props, children, parent, Ctor) {
        ...
        // 确保功能组件中的 createElement 函数获得唯一的上下文 - 这是正确命名插槽检查所必需的
        var contextVm;
        if (hasOwn(parent, '_uid')) { //判断这个组件是否是 new _init  过
            ...
            // in this case we want to make sure we are able to get a hold to the real context instance.
            // 传入的上下文 vm 也是一个函数上下文。在这种情况下,我们要确保我们能够掌握真实的上下文实例。
            contextVm = parent;
            // $flow-disable-line
...
// v-once
    ...
    function genInlineTemplate(el, state) {
        ...
        if (ast.type === 1) {
            ...
            return ("inlineTemplate:{render:function(){" + (inlineRenderFns.render) + "}," +
                "staticRenderFns:[" + (inlineRenderFns.staticRenderFns.map(function (code) {
    ...
    // 1:需要简单的标准化(可能是1级深嵌套数组)
    // 2:需要完全标准化
    // 如果children.length==0 就返回0，如果如果有for属性存在或者tag等于template或者是slot 则为真就返回1，如果是组件则返回2
    ...
    // 返回虚拟dom vonde渲染调用的函数  componentName  --组件名称
    ...
    // 这些关键字不应出现在表达式中，但允许使用 typeof、instanceof 和 in 等运算符
...
// these unary operators should not be used as property/method names 这些一元运算符不应该用作属性/方法名
    ...
    function checkNode(node, errors) {
        ...
        if (node.type === 1) { //text 节点类型，相当于在dom点中的空白区域
            ...
            for (var name in node.attrsMap) {
                //  var dirRE = /^v-|^@|^:/;  判断属性开头是否为 v- @ : 等 //如果是vue 中的属性则抽离出来
                if (dirRE.test(name)) {
    ...
    */
    ...
    /*
     ...
     Description： 函数科里化,创建一个对象,并且把字符串转换成对象函数方式存在在对象中,导出匿名函数
     Calls:
     Called By: 调用本函数的清单
    ...
    function createCompileToFunctionFn(compile) {
        ...
        //创建一个空的对象
        var cache = Object.create(null);
        // 函数科里化,把字符串编译变成真正的js,并且以对象函数方式导出去
        return function compileToFunctions(template, options, vm) {
            ...
            // this should only happen if there is a bug in the compiler itself. mostly for codegen development use
            // 检查函数生成错误。只有在编译器本身存在错误时才会发生这种情况。主要用于 codegen development 使用
            {
    ...
    // `createCompilerCreator` 允许创建使用替代 parser/optimizer/codegen 的编译器，例如 SSR 优化编译器。在这里，我们只是使用默认部分导出默认编译器
    //编译器创建的创造者
    var createCompiler = createCompilerCreator(
        // 把html变成ast模板对象，然后再转换成虚拟dom渲染的函数参数形式。
        // 返回出去一个对象
        // {ast: ast, //ast 模板
    ...
    Vue.prototype.$mount = function (el, hydrating) { //重写Vue.prototype.$mount
        ...
        if (!options.render) {
            ...
            if (template) {
                ...
                } else if (template.nodeType) { // 如果template 是dom节点 则获取他的html
     ...
     * 获取元素的 outerHTML，同时处理 IE 中的 SVG 元素
